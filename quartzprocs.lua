		...
		if not spellCustomOptions then
			spellCustomOptions = {
				...
				cDebuff = {
					type = 'toggle',
					name = 'Debuff',
					desc = 'Is this a debuff',
					get = function(info)
						local sKey = info[3]
						if (db.spells[sKey].debuff == true) then
							return true
						end
						return false
					end,
					set = function(info, value)
						local sKey = info[3]
						if (db.spells[sKey] ~= nil) then
							db.spells[sKey].debuff = value
							if (value == false) then
								db.spells[sKey].debuff = nil
							end
						end
					end,
					order = 41,
			...
			spellCLCustomOptions = {
				...
				cSpellID = {
					...
					set = function(info, value)
						...
						print("Spell ID changed to " .. value)
						
						db.spells[value] = db.spells[sKey]
						options.args.customprocs.args[value] = options.args.customprocs.args[sKey]
						procs[tostring(value)] = procs[tostring(sKey)]
